...
export const normalizeDataset = (data, maxValue = 0, { minX, maxX, minY, maxY }) => {
  ...
  const normalizedMax = Math.max(...data);
  const boundariesY = { min: Math.min(...data), max: Math.max(normalizedMax, maxValue) };
